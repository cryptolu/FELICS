...
/*  Extract and apply the subtweakey to the internal state (must be the two top
 * rows XORed together), then update the tweakey state */
...
void AddKeyPrecomputed(uint8_t state[4][4], uint8_t *roundKeys, int it, int ver) {
    ...
    for (i = 0; i <= 1; i++) {
        for (j = 0; j < 4; j++) {
            if (j % 2 == 0)
                state[i][j] ^= (roundKeys[(i * 4 + j) >> 1]) & 0xF;
            else
                state[i][j] ^= (roundKeys[(i * 4 + j) >> 1] >> 4) & 0xF;
        }
    }
}

/*  Extract and apply the subtweakey to the internal state (must be the two top
 * rows XORed together), then update the tweakey state (inverse function} */
...
/*  Apply the constants: using a LFSR counter on 6 bits, we XOR the 6 bits to
 * the first 6 bits of the internal state */
