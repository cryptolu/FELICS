...
void Encrypt(uint8_t *block, uint8_t *roundKeys) {
    ...
    for (i = 0; i < 16; i++) {  /*loads plaintext into state, then loads keys into keycell */
        if (versions[ver][0] == 64) {
            if (i & 1) {
                ...
                keyCells[1][i>>2][i&0x3] = roundKeys[(i+16)>>1]&0xF;
                if (versions[ver][1]>=192)
                keyCells[2][i>>2][i&0x3] = roundKeys[(i+32)>>1]&0xF;
#endif  /*NOKEYSCHEDULE*/
            } else {
                ...
                if (versions[ver][1]>=128)
                keyCells[1][i>>2][i&0x3] = (roundKeys[(i+16)>>1]>>4)&0xF;
                if (versions[ver][1]>=192)
                keyCells[2][i>>2][i&0x3] = (roundKeys[(i+32)>>1]>>4)&0xF;
#endif  /*NOKEYSCHEDULE*/
            }
    ...
    for (i = 0; i < versions[ver][2]; i++) {    /*Actual encryption, n rounds*/
        if (versions[ver][0] == 64)
            SubCell4(state);
        else
            SubCell8(state);
...
#else
        AddKeyPrecomputed(state, roundKeys + 4 * i, i, ver);    /*32bits of key added*/
#endif  /*NOKEYSCHEDULE*/
        ShiftRows(state);
        MixColumn(state);
    }                           /*The last subtweakey should not be added*/

    if (versions[ver][0] == 64) {   /*if block size */
        for (i = 0; i < 8; i++)
            block[i] =
                    ((state[(2 * i) >> 2][(2 *
                                            i) & 0x3] & 0xF) << 4) | (state[(2 *
                                    i + 1) >> 2][(2 * i + 1) & 0x3] & 0xF);
    } else if (versions[ver][0] == 128) {   /*if block size */
        for (i = 0; i < 16; i++)
