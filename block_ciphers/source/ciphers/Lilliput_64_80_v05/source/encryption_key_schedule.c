...
void RunEncryptionKeySchedule(uint8_t *key, uint8_t *roundKeys)
{
  ...
  for(i = 0 ; i < 29 ; i++)
  {
    /* ExtractRoundKey */ 
		...
		roundKeys[i*8 + 7] = READ_SBOX_BYTE(Sconcat8[tmpKey[18] & 0x8][tmpKey[13] & 0x8][tmpKey[9] & 0x8][tmpKey[3] & 0x8]) ; 
		roundKeys[i*8 + 6] = READ_SBOX_BYTE(Sconcat4[tmpKey[18] & 0x4][tmpKey[13] & 0x4][tmpKey[9] & 0x4][tmpKey[3] & 0x4]);
		
		roundKeys[i*8 + 5] = READ_SBOX_BYTE(Sconcat2[tmpKey[18] & 0x2][tmpKey[13] & 0x2][tmpKey[9] & 0x2][tmpKey[3] & 0x2]); 
		roundKeys[i*8 + 4] = READ_SBOX_BYTE(Sconcat1[tmpKey[18] & 0x1][tmpKey[13] & 0x1][tmpKey[9] & 0x1][tmpKey[3] & 0x1]);
		
		roundKeys[i*8 + 3] = READ_SBOX_BYTE(Sconcat8[tmpKey[16] & 0x8][tmpKey[10] & 0x8][tmpKey[6] & 0x8][tmpKey[1] & 0x8]); 
		roundKeys[i*8 + 2] = READ_SBOX_BYTE(Sconcat4[tmpKey[16] & 0x4][tmpKey[10] & 0x4][tmpKey[6] & 0x4][tmpKey[1] & 0x4]);
		
		roundKeys[i*8 + 1] = READ_SBOX_BYTE(Sconcat2[tmpKey[16] & 0x2][tmpKey[10] & 0x2][tmpKey[6] & 0x2][tmpKey[1] & 0x2]) ; 
		roundKeys[i*8 + 0] =  READ_SBOX_BYTE(Sconcat1[tmpKey[16] & 0x1][tmpKey[10] & 0x1][tmpKey[6] & 0x1][tmpKey[1] & 0x1]);
		
    	roundKeys[i*8 + 7] ^= READ_SBOX_BYTE(Tabi7[i]);
    	roundKeys[i*8 + 6] ^= READ_SBOX_BYTE(Tabi6[i]);
