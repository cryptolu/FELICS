...
void Decrypt(uint8_t *block, uint8_t *roundKeys)
{
  ...
  for(i = 29 ; i > 0 ; i--)
  {
    ...
    /* PermutationLayer^-1 */
		...
		block[5] = (block[0]&0x0f) | (block[1]<<4);
		uint8_t temp6 = block[6];
		block[6] = (block[1]&0xf0) | (block[3]>>4) ;
		uint8_t temp7 = block[7];
		block[7] = (block[0]&0xf0) | (block[3]&0x0f);
		
		block[0] = (temp4&0x0f) | (temp6<<4);
		block[1] = (temp4>>4) | (temp7&0xf0);
		block[2] = temp5;
		block[3] = (temp7&0x0f) | (temp6&0xf0);
