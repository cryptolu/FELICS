...
if [ $SCRIPT_ARCHITECTURE_ARM == $SCRIPT_ARCHITECTURE ] ; then
    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $eks_execution_time ] ; then
        eks_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $e_execution_time ] ; then
        e_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $dks_execution_time ] ; then
        dks_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $d_execution_time ] ; then
        d_execution_time=0
    fi
fi
