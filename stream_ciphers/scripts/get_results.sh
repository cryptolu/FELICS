...
#     ./get_results.sh [{-h|--help}] [--version] [{-f|--format}=[0|1|2|3|4|5]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-p|--prefix}='...'] [{-co|--compiler_options}='...'] [{-i|incremental}=[0|1]]
#
#    Options:
#        -h, --help
    ...
    do
        ...
        do
                ...
                do
                    ...
                    old_state_size=${old_line[1]}
                    old_key_size=${old_line[2]}
                    old_iv_size=${old_line[3]}
                    old_version=${old_line[4]}
                    old_compiler_options=${old_line[6]}
                    ...
                    do
                        ...
                        new_state_size=${new_line[1]}
                        new_key_size=${new_line[2]}
                        new_iv_size=${new_line[3]}
                        new_version=${new_line[4]}
                        new_compiler_options=${new_line[6]}

                        if [ $old_cipher == $new_cipher ] && [ $old_state_size == $new_state_size ] && [ $old_key_size == $new_key_size ] && [ $old_iv_size == $new_iv_size ] && [ $old_version == $new_version ]  && [ $old_compiler_options == $new_compiler_options ] ; then
                            found=$TRUE
                            break
                        fi
                    done
                    ...
                    if [ $FALSE -eq $found ] ; then
                        ...
                        implementation=$old_cipher"_"$old_state_size"_"$old_key_size"_"$old_iv_size"_v"$old_version
                        found=$FALSE
                ...
                do
                    ...
                    implementation=$new_cipher"_"$new_state_size"_"$new_key_size"_"$new_iv_size"_v"$new_version
                    found=$FALSE
