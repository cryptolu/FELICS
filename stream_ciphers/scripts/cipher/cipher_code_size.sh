...
#     ./cipher_code_size.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-b|build}=[0|1]] [{-co|--compiler_options}='...']
#
...
#        ./cipher_code_size.sh -t=./../../source/ciphers/CipherName_StateSizeInBits_KeySizeInBits_IVSizeInBits_v01/build
#
...
if [ $SCRIPT_MODE_0 -ne $SCRIPT_MODE ] ; then
    ...
    do
        ...
        do
            ...
            case $code_section in
                $CODE_SECTION_S)
                    shared_code_s=$(($shared_code_s + $shared_value))
                    ;;
                $CODE_SECTION_E)
    ...
    # Cipher
    cipher_s=$(($setup_rom + $shared_code_s + $shared_constants_s))
    cipher_e=$(($encrypt_rom + $shared_code_e + $shared_constants_e))
    cipher_total=$(($setup_rom + $encrypt_rom + $shared_code_total + $shared_constants_total))

    #Scenarios
    case $SCRIPT_SCENARIO in
        $SCRIPT_SCENARIO_1)
            # Scenario 1
            scenario1_s=$cipher_s
            scenario1_e=$(($encrypt_scenario1_rom + $cipher_e))
            scenario1_total=$(($encrypt_scenario1_rom + $cipher_total))
            ;;
    esac
fi
...
if [ $SCRIPT_MODE_0 -eq $SCRIPT_MODE ] ; then
    ...
    case $SCRIPT_SCENARIO in
        $SCRIPT_SCENARIO_0)
            # Display results
            printf "%s %s %s" $cipher_s $cipher_e $cipher_total > $SCRIPT_OUTPUT
            ;;
        $SCRIPT_SCENARIO_1)
            # Display results
            printf "%s %s %s" $scenario1_s $scenario1_e $scenario1_total > $SCRIPT_OUTPUT
            ;;
    esac
fi
