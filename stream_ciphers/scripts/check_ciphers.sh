...
#     ./check_ciphers.sh [{-h|--help}] [--version] [{-v|--verbosity}=[0|1|2]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-co|--compiler_options}='...']
#
#    Options:
#        -h, --help
...
#        ./check_ciphers.sh -a='PC AVR' --scenarios="0 1"
#
...
do
    ...
    # Get the cipher name, key size, IV size and implementation version from directory name
    cipher_name=$(echo $directory| cut -d $DIRECTORY_NAME_SEPARATOR -f 1)
    cipher_state_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 2)
    cipher_key_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 3)
    cipher_iv_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 4)
