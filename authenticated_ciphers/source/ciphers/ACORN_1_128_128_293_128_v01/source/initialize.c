...
void Initialize(uint8_t *state, const uint8_t *key, const uint8_t *nonce)
{
    ...
    for (j = 0;  j < 16;  j = j+4)
    {
        acorn128_32steps(state, (uint8_t *)&(key[j]), tem, 0xff, 0xff, 1);
    }

    //load the nonce
    for (j = 16;  j < 32;  j = j+4)
    {
        acorn128_32steps(state, (uint8_t *)&(nonce[j - 16]), tem, 0xff, 0xff, 1);
    }
    ...
    {
        acorn128_32steps(state, (uint8_t *)&(key[j&0xf]), tem, 0xff, 0xff, 1);
    }
}
