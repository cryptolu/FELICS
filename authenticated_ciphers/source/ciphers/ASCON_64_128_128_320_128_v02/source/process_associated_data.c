...
void ProcessAssociatedData(uint8_t *state, uint8_t *associatedData,
    ...
    // process associated data - ASCON Team code
    if (associated_data_length) {
        rlen = associated_data_length;
        while (rlen >= RATE) {
            COMPRESS_BYTE_ARRAY(associatedData, in_o, in_e);
            x0_e ^= in_e;
            x0_o ^= in_o;
            P6_32;
            rlen -= RATE;
        ...
        t1_e = 0;
        t1_o = 0;
        for (i = 0; i < rlen; ++i, ++associatedData)
            if (i < 4)
                t1_o |= INS_BYTE32(*associatedData, i);
            else
                t1_e |= INS_BYTE32(*associatedData, (i - 4));
        if (rlen < 4)
