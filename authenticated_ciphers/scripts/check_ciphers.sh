...
#     ./check_ciphers.sh [{-h|--help}] [--version] [{-v|--verbosity}=[0|1|2]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1 2 3 4']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-co|--compiler_options}='...']
#
#    Options:
#        -h, --help
...
#        ./check_ciphers.sh -a='PC AVR' --scenarios="1 2 3 4 5 6"
#
...
do
    ...
    cipher_nonce_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 4)
    cipher_state_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 5)
    cipher_tag_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 6)
    cipher_implementation_version=$(echo $directory| cut -d $DIRECTORY_NAME_SEPARATOR -f 7)
