...
#     ./get_results.sh [{-h|--help}] [--version] [{-f|--format}=[0|1|2|3|4|5]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1 2 3 4 5 6']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-p|--prefix}='...'] [{-co|--compiler_options}='...'] [{-i|incremental}=[0|1]]
#
#    Options:
#        -h, --help
...
#        -i, --incremental
#            Specifies if script should use an incremental strategy (collect results just for new and modified implementations since the last run)
#                0 - do not use incremental strategy
    ...
    do
        ...
        do
                ...
                do
                    ...
                    old_nonce_size=${old_line[3]}
                    old_state_size=${old_line[4]}
                    old_tag_size=${old_line[5]}
                    old_version=${old_line[6]}
                    old_compiler_options=${old_line[7]}
                    ...
                    do
                        ...
                        new_nonce_size=${old_line[3]}
                        new_state_size=${old_line[4]}
                        new_tag_size=${old_line[5]}
                        new_version=${old_line[6]}
                        new_compiler_options=${old_line[7]}

                        if [ $old_cipher == $new_cipher ] && [ $old_block_size == $new_block_size ] && [ $old_key_size == $new_key_size ] && [ $old_nonce_size == $new_nonce_size ] && [ $old_state_size == $new_state_size ] && [ $old_tag_size == $new_tag_size ] && [ $old_version == $new_version ]  && [ $old_compiler_options == $new_compiler_options ] ; then
                            found=$TRUE
                            break
                        fi
                    done
                    ...
                    if [ $FALSE -eq $found ] ; then
                        ...
                        implementation=$old_cipher"_"$old_block_size"_"$old_key_size"_"$old_nonce_size"_"$old_state_size"_"$old_tag_size"_v"$old_version
                        found=$FALSE
                ...
                do
                    ...
                    implementation=$new_cipher"_"$new_block_size"_"$new_key_size"_"$new_nonce_size"_"$new_state_size"_"$new_tag_size"_v"$new_version
                    found=$FALSE
