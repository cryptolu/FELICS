...
#     ./collect_ciphers_metrics.sh [{-h|--help}] [--version] [{-f|--format}=[0|1|2|3|4|5]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1 2 3 4 5 6']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-co|--compiler_options}='...']
#
#    Options:
#        -h, --help
...
#                List of values: '0 1 2 3 4 5 6'
#                Default: all scenarios
#        -c, --ciphers
#            Specifies for which ciphers to collect the metrics
#                List of values: 'CipherName_BlockSizeInBits_KeySizeInBits_NonceSizeInBits_StateSizeInBits_TagSizeInBits_v01 ...'
#                Default: all ciphers
#        -co,--compiler_options
...
do
    ...
    do
        ...
        do
            ...
            cipher_nonce_size=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 4)
            cipher_state_size=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 5)
            cipher_tag_size=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 6)
            cipher_implementation_version=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 7)
            ...
            do
                ...
                echo -e "\t NONCE_SIZE = $cipher_nonce_size"
                echo -e "\t STATE_SIZE = $cipher_state_size"
                echo -e "\t TAG_SIZE = $cipher_tag_size"
                ...
                case $SCRIPT_FORMAT in
                    $SCRIPT_FORMAT_0)
                        add_raw_table_row $script_raw_output $scenario $cipher_name $cipher_block_size $cipher_key_size $cipher_nonce_size $cipher_state_size $cipher_tag_size $cipher_implementation_version $cipher_implementation_language "$compiler_option" ${values[@]}
                        add_mediawiki_table_row $script_mediawiki_output $scenario $cipher_name $cipher_block_size $cipher_key_size $cipher_nonce_size $cipher_state_size $cipher_tag_size $cipher_implementation_version $cipher_implementation_language "$compiler_option" ${values[@]}
                        add_xml_table_row $script_xml_output $scenario $cipher_name $cipher_block_size $cipher_key_size $cipher_nonce_size $cipher_state_size $cipher_tag_size $cipher_implementation_version $cipher_implementation_language "$compiler_option" ${values[@]}
                        add_latex_table_row $script_latex_output $scenario $cipher_name $cipher_block_size $cipher_key_size $cipher_nonce_size $cipher_state_size $cipher_tag_size $cipher_implementation_version $cipher_implementation_language "$compiler_option" ${values[@]}
                        add_csv_table_row $script_csv_output $scenario $cipher_name $cipher_block_size $cipher_key_size $cipher_nonce_size $cipher_state_size $cipher_tag_size $cipher_implementation_version $cipher_implementation_language "$compiler_option" ${values[@]}
                        ;;
                    $SCRIPT_FORMAT_1)
                        add_raw_table_row $script_raw_output $scenario $cipher_name $cipher_block_size $cipher_key_size $cipher_nonce_size $cipher_state_size $cipher_tag_size $cipher_implementation_version $cipher_implementation_language "$compiler_option" ${values[@]}
                        ;;
