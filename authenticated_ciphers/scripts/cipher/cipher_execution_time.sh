...
#     ./cipher_execution_time.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1|2|3|4|5|6]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-b|build}=[0|1]] [{-co|--compiler_options}='...']
#
...
#        ./cipher_execution_time.sh -t=./../../source/ciphers/CipherName_BlockSizeInBits_KeySizeInBits_NonceSizeInBits_StateSizeInBits_TagSizeInBits_v01/build
#
...
function simulate()
{
    ...
    case $SCRIPT_ARCHITECTURE in
        $SCRIPT_ARCHITECTURE_PC)
            ...
            echo $target_file >> target.log
            echo $output_file >> target.log
            ;;
        ...
        $SCRIPT_ARCHITECTURE_ARM)
            ...
            if [ $SCRIPT_SCENARIO_0 -eq $SCRIPT_SCENARIO ]; then
                make -f $CIPHER_MAKEFILE ARCHITECTURE=$SCRIPT_ARCHITECTURE SCENARIO=$SCRIPT_SCENARIO upload-cipher &> $make_log_file
            else
                make -f $CIPHER_MAKEFILE ARCHITECTURE=$SCRIPT_ARCHITECTURE SCENARIO=$SCRIPT_SCENARIO upload-scenario &> $make_log_file
            fi
...
function compute_execution_time()
{
    ...
    case $SCRIPT_ARCHITECTURE in
        $SCRIPT_ARCHITECTURE_PC)
            ...
            ;;
        $SCRIPT_ARCHITECTURE_AVR)
            local initial_value=$(cat $output_file | grep -e "--(CALL)-> $first_row_identifier" | grep "$first_row_identifier$" | head -1 | tr -d '\r' | tr -s ' ' | cut -d ' ' -f 3)

            local final_value
            if [ -z $second_row_identifier ] ; then
                final_value=$(cat $output_file | grep -e "<-(RET )-- $first_row_identifier" | grep "$first_row_identifier$" | head -1 | tr -d '\r' | tr -s ' ' | cut -d ' ' -f 3)
            else
                final_value=$(cat $output_file | grep -e "--(CALL)-> $second_row_identifier" | grep "$second_row_identifier$" | head -1 | tr -d '\r' | tr -s ' ' | cut -d ' ' -f 3)
            fi
...
ini_execution_time=0
pad_execution_time=0
ppd_execution_time=0
fin_execution_time=0
tg_execution_time=0
pcd_execution_time=0
tv_execution_time=0
...
case $SCRIPT_ARCHITECTURE in
    $SCRIPT_ARCHITECTURE_PC)
        ...
        case $SCRIPT_SCENARIO in
            ...
            $SCRIPT_SCENARIO_3)
                ...
                ;;
            $SCRIPT_SCENARIO_4)
                simulate $PC_SCENARIO4_FILE $pc_output_file $make_log_file
                ;;
            $SCRIPT_SCENARIO_5)
                simulate $PC_SCENARIO5_FILE $pc_output_file $make_log_file
                ;;
            $SCRIPT_SCENARIO_6)
                simulate $PC_SCENARIO6_FILE $pc_output_file $make_log_file
                ;;
        esac

        if [ -f $pc_output_file ] ; then
            ini_execution_time=$(compute_execution_time $pc_output_file 'EncryptionInitializationCycleCount')
            pad_execution_time=$(compute_execution_time $pc_output_file 'EncryptionProcessAssociatedDataCycleCount')
            ppd_execution_time=$(compute_execution_time $pc_output_file 'ProcessPlaintextCycleCount')
            fin_execution_time=$(compute_execution_time $pc_output_file 'EncryptionFinalizationCycleCount')
            tg_execution_time=$(compute_execution_time $pc_output_file 'TagGenerationCycleCount')
            pcd_execution_time=$(compute_execution_time $pc_output_file 'ProcessCiphertextCycleCount')
            tv_execution_time=$(compute_execution_time $pc_output_file 'TagVerificationCycleCount')
        fi
    ...
    $SCRIPT_ARCHITECTURE_AVR)
        ...
        if [ -f $avr_execution_time_log_file ] ; then
            ini_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Initialize' 'EndEncryptionInitialization')
            pad_execution_time=$(compute_execution_time $avr_execution_time_log_file 'ProcessAssociatedData' 'EndEncryptionAssociatedDataProcessing')
            ppd_execution_time=$(compute_execution_time $avr_execution_time_log_file 'ProcessPlaintext' 'EndPlaintextProcessing')
            pcd_execution_time=$(compute_execution_time $avr_execution_time_log_file 'ProcessCiphertext' 'EndCiphertextProcessing')
            fin_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Finalize' 'EndEncryptionFinalization')
            tg_execution_time=$(compute_execution_time $avr_execution_time_log_file 'TagGeneration' 'EndTagGeneration')
            tv_execution_time=$(compute_execution_time $avr_execution_time_log_file 'TagVerification' 'EndTagVerification')
    ...
    $SCRIPT_ARCHITECTURE_MSP)
        ...
        case $SCRIPT_SCENARIO in
            ...
            $SCRIPT_SCENARIO_3)
                ...
                ;;
            $SCRIPT_SCENARIO_4)
                simulate $MSP_SCENARIO4_MSPDEBUG_EXECUTION_TIME_COMMANDS_FILE $mspdebug_execution_time_log_file
                simulate $MSP_SCENARIO4_MSPDEBUG_EXECUTION_TIME_SECTIONS_COMMANDS_FILE $mspdebug_execution_time_sections_log_file
                ;;
            $SCRIPT_SCENARIO_5)
                simulate $MSP_SCENARIO5_MSPDEBUG_EXECUTION_TIME_COMMANDS_FILE $mspdebug_execution_time_log_file
                simulate $MSP_SCENARIO5_MSPDEBUG_EXECUTION_TIME_SECTIONS_COMMANDS_FILE $mspdebug_execution_time_sections_log_file
                ;;
            $SCRIPT_SCENARIO_6)
                simulate $MSP_SCENARIO6_MSPDEBUG_EXECUTION_TIME_COMMANDS_FILE $mspdebug_execution_time_log_file
                simulate $MSP_SCENARIO6_MSPDEBUG_EXECUTION_TIME_SECTIONS_COMMANDS_FILE $mspdebug_execution_time_sections_log_file
                ;;
        esac

        if [ -f $mspdebug_execution_time_log_file ] ; then
            ini_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 1)
            pad_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 3)
            ppd_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 5)

            if [ $SCRIPT_SCENARIO -eq $SCRIPT_SCENARIO_1 ] || [ $SCRIPT_SCENARIO -eq $SCRIPT_SCENARIO_4 ] ; then
                pcd_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 7)
                fin_execution_time=0
                tg_execution_time=0
                tv_execution_time=0
            else
                fin_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 7)
                tg_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 9)
                pcd_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 11)
                tv_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 13)
            fi
    ...
    $SCRIPT_ARCHITECTURE_ARM)
        ...
        case $SCRIPT_SCENARIO in
            $SCRIPT_SCENARIO_0)
                simulate $file $arm_serial_terminal_output_file $make_log_file
                ;;
        ...
        if [ -f $arm_serial_terminal_output_file ] ; then
            ini_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'EncryptionInitializationCycleCount')
            pad_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'EncryptionProcessAssociatedDataCycleCount')
            ppd_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'ProcessPlaintextCycleCount')
            fin_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'EncryptionFinalizationCycleCount')
            tg_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'TagGenerationCycleCount')
            pcd_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'ProcessCiphertextCycleCount')
            tv_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'TagVerificationCycleCount')
        fi
...
elif [ $SCRIPT_SCENARIO_1 -eq $SCRIPT_SCENARIO ] || [ $SCRIPT_SCENARIO_4 -eq $SCRIPT_SCENARIO ] ; then

    fin_execution_time=0
    tg_execution_time=0
    tv_execution_time=0

    if [ $PROCESS_EMPTY_ASSOCIATED_DATA_NO == "$process_empty_associated_data" ] ; then
        pad_execution_time=0
    fi

elif [ $SCRIPT_SCENARIO_2 -eq $SCRIPT_SCENARIO ] || [ $SCRIPT_SCENARIO_5 -eq $SCRIPT_SCENARIO ] && [ $PROCESS_EMPTY_MESSAGE_NO = "$process_empty_message" ]; then
...
# Test if ARM mesurements are valid
if [ $SCRIPT_ARCHITECTURE_ARM = $SCRIPT_ARCHITECTURE ] ; then
    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $ini_execution_time ] ; then
        ini_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $pad_execution_time ] ; then
        pad_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $ppd_execution_time ] ; then
        ppd_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $fin_execution_time ] ; then
        fin_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $tg_execution_time ] ; then
        tg_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $pcd_execution_time ] ; then
        pcd_execution_time=0
    fi

    if [ $ARM_OPERATION_MAX_EXECUTION_TIME -le $tv_execution_time ] ; then
        tv_execution_time=0
    fi
fi
...
if [ $SCRIPT_MODE_0 -eq $SCRIPT_MODE ] ; then
    ...
    printf "\n" >> $SCRIPT_OUTPUT
    printf "| %10s | %10s | %10s | %10s | %10s | %10s | %10s | %10s |\n" "Scenario" "Initialize" "PAD" "PPD" "Finalize" "TG" "PCD" "TV" >> $SCRIPT_OUTPUT
    printf "%0.s-" $(seq 1 $TABLE_HORIZONTAL_LINE_LENGTH) >> $SCRIPT_OUTPUT
    printf "\n" >> $SCRIPT_OUTPUT

    # Table line
    printf "| %10s | %10s | %10s | %10s | %10s | %10s | %10s | %10s |\n" $total_execution_time $ini_execution_time $pad_execution_time $ppd_execution_time $fin_execution_time $tg_execution_time $pcd_execution_time $tv_execution_time >> $SCRIPT_OUTPUT

    # Table footer
    printf "%0.s-" $(seq 1 $TABLE_HORIZONTAL_LINE_LENGTH) >> $SCRIPT_OUTPUT
    printf "\n" >> $SCRIPT_OUTPUT
else
    printf "%s %s %s %s %s %s %s" $ini_execution_time $pad_execution_time $ppd_execution_time $fin_execution_time $tg_execution_time $pcd_execution_time $tv_execution_time > $SCRIPT_OUTPUT
fi
