...
#     ./check_cipher.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1|2|3|4|5|6]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-co|--compiler_options}='...']
#
...
#                6 - scenario 2c (scenario 6)#                      0 - cipher and debug scenario
#                      1 - scenario 1a (scenario 1)
#                      2 - scenario 1b (scenario 2)
#                      3 - scenario 1c (scenario 3)
#                      4 - scenario 2a (scenario 4)
...
#        ./check_cipher.sh -t=./../../source/ciphers/CipherName_BlockSizeInBits_KeySizeInBits_NonceSizeInBits_StateSizeInBits_TagSizeInBits_v01/build
#
...
if [ $TRUE == $compliant ] && [ $SCRIPT_SCENARIO_0 == $SCRIPT_SCENARIO ] ; then
    ...
    # Get number of state checks to be skipped
    number_state_check_skips=0
    skip_check_ini=$(cat $CONSTANTS_SOURCE_FILE | grep "SKIP_STATE_CHECK_INI" | tr -d '\r' | cut -d ' ' -f 3)
    skip_check_pad=$(cat $CONSTANTS_SOURCE_FILE | grep "SKIP_STATE_CHECK_PAD" | tr -d '\r' | cut -d ' ' -f 3)
    skip_check_ppd=$(cat $CONSTANTS_SOURCE_FILE | grep "SKIP_STATE_CHECK_PPD" | tr -d '\r' | cut -d ' ' -f 3)
    skip_check_fin=$(cat $CONSTANTS_SOURCE_FILE | grep "SKIP_STATE_CHECK_FIN" | tr -d '\r' | cut -d ' ' -f 3)
    skip_check_pcd=$(cat $CONSTANTS_SOURCE_FILE | grep "SKIP_STATE_CHECK_PCD" | tr -d '\r' | cut -d ' ' -f 3)

    if [ ! -z $skip_check_ini ] && [ $SKIP_STATE_CHECK_TRUE == $skip_check_ini ] ; then
        number_state_check_skips=$((number_state_check_skips + 2))
    fi

    if [ ! -z $skip_check_pad ] && [ $SKIP_STATE_CHECK_TRUE == $skip_check_pad ] ; then
        number_state_check_skips=$((number_state_check_skips + 2))
    fi

    if [ ! -z $skip_check_ppd ] && [ $SKIP_STATE_CHECK_TRUE == $skip_check_ppd ] ; then
        number_state_check_skips=$((number_state_check_skips + 1))
    fi

    if [ ! -z $skip_check_fin ] && [ $SKIP_STATE_CHECK_TRUE == $skip_check_fin ] ; then
        number_state_check_skips=$((number_state_check_skips + 2))
    fi

    if [ ! -z $skip_check_pcd ] && [ $SKIP_STATE_CHECK_TRUE == $skip_check_pcd ] ; then
        number_state_check_skips=$((number_state_check_skips + 1))
    fi
    ...
    if [ -f $RESULT_FILE ] ; then
        ...
        let correct_count=$(($correct_count + $number_state_check_skips))
