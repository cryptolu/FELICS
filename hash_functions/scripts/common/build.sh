...
# 	./build.sh [{-h|--help}] [--version] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-s|--scenario}=[0|1A|1B|1C|2]] [{-v|--verbose}=[0|1]] [{-co|--compiler_options}='...']
#
...
# Build
if [ $SCRIPT_ARCHITECTURE == $SCRIPT_ARCHITECTURE_MSP ] ; then
	make -f $CIPHER_MAKEFILE ARCHITECTURE=$SCRIPT_ARCHITECTURE SCENARIO=$SCRIPT_SCENARIO COMPILER_OPTIONS="$SCRIPT_COMPILER_OPTIONS" DEBUG=1 &> $make_log_file
else
