...
do
	...
	# Get the hash name, block size, state size. no of rounds and implementation version from directory name
	hash_name=$(echo $directory| cut -d $DIRECTORY_NAME_SEPARATOR -f 1)
	hash_block_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 2)
	hash_state_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 3)
	hash_digest_size=$(echo $directory | cut -d $DIRECTORY_NAME_SEPARATOR -f 4)
	cipher_implementation_version=$(echo $directory| cut -d $DIRECTORY_NAME_SEPARATOR -f 5)
		

	if [ $SCRIPT_VERBOSITY_2 -eq $SCRIPT_VERBOSITY ] ; then
		echo ""
		echo "Run for hash '$hash_name':"
	...
	# Assume that the hash is compliant
	compliant=$TRUE
	...
	if [ $TRUE -eq $compliant ] ; then
		...
		do
			...
			do
				...
				# Assume that the hash implementation is not compliant with the architecture and scenario
...
echo ""
echo -e "\t Checked (hash, architecture, scenario) pairs: $checked_cipher_implementation_architecture_scenarios"
echo -e "\t Compliant (hash, architecture, scenario) pairs: $compliant_cipher_implementation_architecture_scenarios"
echo -e "\t Not compliant (hash, architecture, scenario) pairs: $(($checked_cipher_implementation_architecture_scenarios - $compliant_cipher_implementation_architecture_scenarios))"
