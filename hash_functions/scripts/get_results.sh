...
# 	./get_results.sh [{-h|--help}] [--version] [{-f|--format}=[0|1|2|3|4|5]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1 2 3 4']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-p|--prefix}='...'] [{-co|--compiler_options}='...'] [{-i|incremental}=[0|1]]
#
#	Options:
#		-h, --help
	...
	do
		...
		do
				...
				do
					...
					old_state_size=${old_line[2]}
					old_digest_size=${old_line[3]}
					old_version=${old_line[4]}
					...
					do
						...
						new_state_size=${new_line[2]}
						new_digest_size=${new_line[3]}
						new_version=${new_line[4]}
						new_compiler_options=${new_line[5]}

						if [ $old_hash == $new_hash ] && [ $old_digest_size == $new_digest_size ] && [ $old_state_size == $new_state_size ] && [ $old_block_size == $new_block_size ] && [ $old_version == $new_version ]  && [ $old_compiler_options == $new_compiler_options ] ; then
							found=$TRUE
							break
						fi
					done
					...
					if [ $FALSE -eq $found ] ; then
						...
						implementation=$old_hash"_"$old_block_size"_"$old_state_size"_"$old_digest_size"_v"$old_version
						found=$FALSE
				...
				do
					...
					implementation=$new_hash"_"$new_block_size"_"$new_state_size"_"$new_digest_size"_v"$new_version
					found=$FALSE
