...
#
# This script should be called from /authenticated_ciphers/scripts/
# Call this script to collect the ciphers metrics
# 	./collect_ciphers_metrics.sh [{-h|--help}] [--version] [{-f|--format}=[0|1|2|3|4|5]] [{-a|--architectures}=['PC AVR MSP ARM']] [{-s|--scenarios}=['0 1A 1B 1C 2']] [{-c|--ciphers}=['Cipher1 Cipher2 ...']] [{-co|--compiler_options}='...']
#
#	Options:
#		-h, --help
...
#				List of values: '0 1A 1B 1C 2'
#				Default: all scenarios
#		-c, --ciphers
#			Specifies for which ciphers to collect the metrics
#				List of values: 'HashName_BlockSizeInBits_StateSizeInBits_DigestSizeInBits_v01 ...'
#				Default: all ciphers
#		-co,--compiler_options
...
do
	...
	do
		...
		do
			...
			hash_name=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 1)
			hash_block_size=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 2)
			hash_state_size=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 3)
			hash_digest_size=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 4)
			hash_implementation_version=$(echo $cipher_directory_name | cut -d $DIRECTORY_NAME_SEPARATOR -f 5)
			hash_implementation_version=${hash_implementation_version:1:${#hash_implementation_version}-1}

			if [ $EXAMPLE_CIPHER_NAME == $hash_name ] ; then
				cd ./../../
				continue
			fi


			hash_implementation_language=$(cat $IMPLEMENTATION_INFO_FILE | grep $IMPLEMENTATION_TYPE$architecture$SECTION_SEPARATOR | tr -d '\r' | cut -d ':' -f 2 |  tr -d '[[:space:]]')

			# Convert to uppercase
			hash_implementation_language=${hash_implementation_language^^}

			if [ $IMPLEMENTATION_ASM != "$hash_implementation_language" ] && [ $IMPLEMENTATION_C_ASM != "$hash_implementation_language" ] ; then
				hash_implementation_language=$IMPLEMENTATION_C
			fi


			for compiler_option in "${compiler_options[@]}"		
			do
				echo "Run for cipher '$hash_name':"
				echo -e "\t BLOCK_SIZE = $hash_block_size"
				echo -e "\t STATE_SIZE = $hash_state_size"
				echo -e "\t DIGEST_SIZE = $hash_digest_size"
				echo -e "\t IMPLEMENTATION_VERSION = $hash_implementation_version"
				echo -e "\t ARCHITECTURE = $architecture"			
				echo -e "\t SCENARIO = $scenario"
				...
				case $SCRIPT_FORMAT in
					$SCRIPT_FORMAT_0)
						add_raw_table_row $script_raw_output $scenario $hash_name $hash_block_size $hash_state_size $hash_digest_size $hash_implementation_version $hash_implementation_language "$compiler_option" ${values[@]}
						add_mediawiki_table_row $script_mediawiki_output $scenario $hash_name $hash_block_size $hash_state_size $hash_digest_size $hash_implementation_version $hash_implementation_language "$compiler_option" ${values[@]}
						add_xml_table_row $script_xml_output $scenario $hash_name $hash_block_size $hash_state_size $hash_digest_size $hash_implementation_version $hash_implementation_language "$compiler_option" ${values[@]}
						add_latex_table_row $script_latex_output $scenario $hash_name $hash_block_size $hash_state_size $hash_digest_size $hash_implementation_version $hash_implementation_language "$compiler_option" ${values[@]}
						add_csv_table_row $script_csv_output $scenario $hash_name $hash_block_size $hash_state_size $hash_digest_size $hash_implementation_version $hash_implementation_language "$compiler_option" ${values[@]}
						;;
					$SCRIPT_FORMAT_1)
						add_raw_table_row $script_raw_output $scenario $hash_name $hash_block_size $hash_state_size $hash_digest_size $hash_implementation_version $hash_implementation_language "$compiler_option" ${values[@]}
						;;
