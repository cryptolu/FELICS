...
do
	...
	hash_implementation_info=$(cat $IMPLEMENTATION_INFO_FILE | grep $IMPLEMENTATION_DESCRIPTION$SECTION_SEPARATOR | tr -d '\r' | cut -d ':' -f 2)
	hash_implementation_authors=$(cat $IMPLEMENTATION_INFO_FILE | grep $IMPLEMENTATION_AUTHORS$SECTION_SEPARATOR | tr -d '\r' | cut -d ':' -f 2)
	...
	case $SCRIPT_FORMAT in
		$SCRIPT_FORMAT_0)
			add_raw_table_row $script_raw_output "$hash_name" "$hash_block_size" "$hash_state_size" "$hash_digest_size" "$hash_implementation_version" "$hash_implementation_info" "$hash_implementation_authors"
			add_mediawiki_table_row $script_mediawiki_output "$hash_name" "$hash_block_size" "$hash_state_size" "$hash_digest_size" "$hash_implementation_version" "$hash_implementation_info" "$hash_implementation_authors"
			add_xml_table_row $script_xml_output "$hash_name" "$hash_block_size" "$hash_state_size" "$hash_digest_size" "$hash_implementation_version" "$hash_implementation_info" "$hash_implementation_authors"
			add_latex_table_row $script_latex_output "$hash_name" "$hash_block_size" "$hash_state_size" "$hash_digest_size" "$hash_implementation_version" "$hash_implementation_info" "$hash_implementation_authors"
			add_csv_table_row $script_csv_output "$hash_name" "$hash_block_size" "$hash_state_size" "$hash_digest_size" "$hash_implementation_version" "$hash_implementation_info" "$hash_implementation_authors"
			;;
		$SCRIPT_FORMAT_1)
			add_raw_table_row $script_raw_output "$hash_name" "$hash_block_size" "$hash_state_size" "$hash_digest_size" "$hash_implementation_version" "$hash_implementation_info" "$hash_implementation_authors"
			;;
