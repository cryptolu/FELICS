...
#
# Call this script to extract the hash code size
# 	./cipher_code_size.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1A|1B|1C|2]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-b|build}=[0|1]] [{-co|--compiler_options}='...']
#
...
#		./cipher_code_size.sh -t=./../../source/ciphers/HashName_BlockSizeInBits_StateSizeInBits_DigestSizeInBits_v01/build
#
...
# Get the cipher name
hash_name=$(basename -- "$(dirname -- "$(pwd)")")
...
if [ 0 -eq $files_number ] ; then
	echo "There is no file matching the pattern: '$pattern' for cipher '$hash_name'!"
	echo "Exit!"
	exit
fi
...
if [ $SCRIPT_MODE_0 -ne $SCRIPT_MODE ] ; then
		...
		do
			...
			case $code_section in
				$CODE_SECTION_INI)
					...
					;;
				$CODE_SECTION_UPD)
					shared_code_upd=$(($shared_code_upd + $shared_value))
					;;
				$CODE_SECTION_FIN)
	...
	cipher_upd=$(($update_rom + $shared_code_upd + $shared_constants_upd))
	cipher_fin=$(($finalize_rom + $shared_code_fin + $shared_constants_fin))
	cipher_total=$(($initialize_rom + $update_rom + $finalize_rom + $shared_code_total + $shared_constants_total))

	#Scenarios
	if [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1A ] || 
	   [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1B ] ||
	   [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1C ] ; then
		# Scenario 1
		scenario1_ini=$cipher_ini
		scenario1_upd=$(($update_scenario1_rom + $cipher_upd))
		scenario1_fin=$cipher_fin
		scenario1_total=$(($update_scenario1_rom + $cipher_total))

	elif [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_2 ] ; then

		if [ $sponge == "TRUE" ] ; then
			scenario2_ini=$cipher_ini
			scenario2_upd=$(($update_scenario2_pmac_rom + $cipher_upd))
			scenario2_fin=$cipher_fin
			scenario2_total=$(($update_scenario2_pmac_rom + $cipher_total))

		elif [ $sponge == "FALSE" ] ; then
			scenario2_ini=$cipher_ini
			scenario2_upd=$(($update_scenario2_hmac_rom + $cipher_upd))
			scenario2_fin=$cipher_fin
			scenario2_total=$(($update_scenario2_hmac_rom + $cipher_total))
...
if [ $SCRIPT_MODE_0 -eq $SCRIPT_MODE ] ; then
	...
	if [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_0 ] ; then
		# Scenario0
		printf "%s %s %s %s" $cipher_ini $cipher_upd $cipher_fin $cipher_total > $SCRIPT_OUTPUT
	elif [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1A ] || 
	   [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1B ] ||
	   [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1C ] ; then
		# Scenario 1
		printf "%s %s %s %s" $scenario1_ini $scenario1_upd $scenario1_fin $scenario1_total > $SCRIPT_OUTPUT
	elif [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_2 ] ; then
		# Scenario 2
		printf "%s %s %s %s" $scenario2_ini $scenario2_upd $scenario2_fin $scenario2_total > $SCRIPT_OUTPUT
	fi
fi
