...
#
# Call this script to check if the hash implementation is compliant with the framework
# 	./check_cipher.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1|2|3|4]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-co|--compiler_options}='...']
#
...
#		./check_cipher.sh -t=./../../source/ciphers/HashName_BlockSizeInBits_StateSizeInBits_DigestSizeInBits_v01/build
#
...
if [ $TRUE == $compliant ] && [ $SCRIPT_SCENARIO_0 == $SCRIPT_SCENARIO ] ; then
	...
	case $SCRIPT_ARCHITECTURE in
		...
		$SCRIPT_ARCHITECTURE_MSP)
			# Run
			if [ -f $CIPHER_ELF_FILE ] ; then
				#/opt/msp_tools/mspdebug/mspdebug-0.24/mspdebug -n sim
    ...
    skip_check_upd=$(cat $CONSTANTS_SOURCE_FILE | grep "SKIP_STATE_CHECK_UPD" | tr -d '\r' | cut -d ' ' -f 3)
    ...
    if [ ! -z $skip_check_upd ] && [ $SKIP_STATE_CHECK_TRUE == $skip_check_upd ] ; then
        number_state_check_skips=$((number_state_check_skips + 1))
    fi
