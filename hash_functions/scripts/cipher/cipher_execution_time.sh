...
# 	./cipher_execution_time.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1A|1B|1C|2]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-b|build}=[0|1]] [{-co|--compiler_options}='...']
#
...
#		./cipher_execution_time.sh -t=./../../source/ciphers/HashName_BlockSizeInBits_StateSizeInBits_DigestSizeInBits_v01/build
#
	...
	case $SCRIPT_ARCHITECTURE in
		...
		$SCRIPT_ARCHITECTURE_MSP)
			case $SCRIPT_SCENARIO in
				$SCRIPT_SCENARIO_0)
					cat $target_file | sed "s/PROG_ELF_FILE/prog $CIPHER_FILE$ELF_FILE_EXTENSION/" | $MSPDEBUG_SIMULATOR -n sim &> $output_file
					;;
				$SCRIPT_SCENARIO_1A)
					cat $target_file | sed "s/PROG_ELF_FILE/prog $SCENARIO1A_FILE$ELF_FILE_EXTENSION/" | $MSPDEBUG_SIMULATOR -n sim &> $output_file
					;;
				$SCRIPT_SCENARIO_1B)
					cat $target_file | sed "s/PROG_ELF_FILE/prog $SCENARIO1B_FILE$ELF_FILE_EXTENSION/" | $MSPDEBUG_SIMULATOR -n sim &> $output_file
					;;
				$SCRIPT_SCENARIO_1C)
					cat $target_file | sed "s/PROG_ELF_FILE/prog $SCENARIO1C_FILE$ELF_FILE_EXTENSION/" | $MSPDEBUG_SIMULATOR -n sim &> $output_file
					;;
				$SCRIPT_SCENARIO_2)
					cat $target_file | sed "s/PROG_ELF_FILE/prog $SCENARIO2_FILE$ELF_FILE_EXTENSION/" | $MSPDEBUG_SIMULATOR -n sim &> $output_file
					;;
			esac
			;;
...
function compute_execution_time()
{
	...
	#echo $first_row_identifier $second_row_identifier
...
case $SCRIPT_ARCHITECTURE in
	$SCRIPT_ARCHITECTURE_PC)
		...
		case $SCRIPT_SCENARIO in
			$SCRIPT_SCENARIO_0)
				...
				;;
			$SCRIPT_SCENARIO_1A)
				simulate $PC_SCENARIO1A_FILE $pc_output_file $make_log_file
				;;
			$SCRIPT_SCENARIO_1B)
				simulate $PC_SCENARIO1B_FILE $pc_output_file $make_log_file
				;;
			$SCRIPT_SCENARIO_1C)
				simulate $PC_SCENARIO1C_FILE $pc_output_file $make_log_file
				;;
			$SCRIPT_SCENARIO_2)
		...
		if [ -f $pc_output_file ] ; then
			ini_execution_time=$(compute_execution_time $pc_output_file 'InitializationCycleCount')
			upd_execution_time=$(compute_execution_time $pc_output_file 'UpdateCycleCount')
			fin_execution_time=$(compute_execution_time $pc_output_file 'FinalizationCycleCount')
		fi
	...
	$SCRIPT_ARCHITECTURE_AVR)
		...
		if 	[ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_0 ] ; then 
			if [ -f $avr_execution_time_log_file ] ; then
				ini_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Initialize' 'EndInitialization')
				upd_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Update' 'EndUpdate')
				fin_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Finalize' 'EndFinalization')
				total_execution_time=$(compute_execution_time $avr_execution_time_log_file 'main')
			fi

		elif [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1A ] || 
			 [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1B ] ||
			 [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_1C ] ; then
			if [ -f $avr_execution_time_log_file ] ; then
				ini_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Initialize' 'EndInitialization')
				upd_execution_time=$(compute_execution_time $avr_execution_time_log_file 'UpdateScenario1' 'EndUpdate')
				fin_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Finalize' 'EndFinalization')
				total_execution_time=$(compute_execution_time $avr_execution_time_log_file 'main')
			fi
		elif [ $SCRIPT_SCENARIO == $SCRIPT_SCENARIO_2 ] ; then
			if [ -f $avr_execution_time_log_file ] ; then
				ini_execution_time=$(compute_execution_time $avr_execution_time_log_file 'Initialize' 'EndInitialization')
				if [ $sponge == "TRUE" ] ; then
					upd_execution_time=$(compute_execution_time $avr_execution_time_log_file 'UpdateScenario2PMAC' 'EndUpdate')
				else
					upd_execution_time=$(compute_execution_time $avr_execution_time_log_file 'UpdateScenario2HMAC' 'EndUpdate')
				fi
				fin_execution_time=$(compute_execution_time $avr_execution_time_log_file 'BeginFinalization' 'EndFinalization')
				total_execution_time=$(compute_execution_time $avr_execution_time_log_file 'main')
			fi
		fi		
	...
	$SCRIPT_ARCHITECTURE_MSP)
		...
		if [ -f $mspdebug_execution_time_log_file ] ; then
			...
			upd_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 3)
			fin_execution_time=$(compute_execution_time $mspdebug_execution_time_sections_log_file 5)
	...
	$SCRIPT_ARCHITECTURE_ARM)
		...
		case $SCRIPT_SCENARIO in
			...
			$SCRIPT_SCENARIO_1A)
				simulate $file $arm_serial_terminal_output_file $make_log_file $UPLOAD_SCENARIO1A
				;;

			$SCRIPT_SCENARIO_1B)
				simulate $file $arm_serial_terminal_output_file $make_log_file $UPLOAD_SCENARIO1B
				;;

			$SCRIPT_SCENARIO_1C)
				simulate $file $arm_serial_terminal_output_file $make_log_file $UPLOAD_SCENARIO1C
				;;
		...
		if [ -f $arm_serial_terminal_output_file ] ; then
			ini_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'InitializationCycleCount')
			upd_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'UpdateCycleCount')
			fin_execution_time=$(compute_execution_time $arm_serial_terminal_output_file 'FinalizationCycleCount')
		fi
...
if [ $SCRIPT_MODE_0 -eq $SCRIPT_MODE ] ; then
	...
	printf "\n" >> $SCRIPT_OUTPUT
	printf "| %10s | %10s | %10s | %10s |\n" "Scenario" "Initialize" "Update" "Finalize" >> $SCRIPT_OUTPUT
	printf "%0.s-" $(seq 1 $TABLE_HORIZONTAL_LINE_LENGTH) >> $SCRIPT_OUTPUT
	printf "\n" >> $SCRIPT_OUTPUT

	# Table line
	printf "| %10s | %10s | %10s | %10s |\n" $total_execution_time $ini_execution_time $upd_execution_time $fin_execution_time >> $SCRIPT_OUTPUT

	# Table footer
	printf "%0.s-" $(seq 1 $TABLE_HORIZONTAL_LINE_LENGTH) >> $SCRIPT_OUTPUT
	printf "\n" >> $SCRIPT_OUTPUT
else
	printf "%s %s %s " $ini_execution_time $upd_execution_time $fin_execution_time > $SCRIPT_OUTPUT
fi
